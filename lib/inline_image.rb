...
class InlineImage
  ...
  IMAGE_SIZE_MAXIMUM_HEIGHT = 250
  IMAGE SIZE MAXIMUM_WIDTH = 250
  ...
  def call(env)
    ...
    (doc/"img").each do |img|
      img.attributes['src'] = encodify_image(img.attributes['src'], request.url.match(/\A(http:\/\/|https:\/\/)/)[1] + request.host_with_port) if should_encodify?(img)
    end
  ...
  def should_encodify?(image)
    return image.attributes['class'] =~ /no-encode/
  end
